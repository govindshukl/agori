...
from agori import SecureChromaDB, AgoriException, ConfigurationError, ProcessingError, SearchError
...
def secure_db(encryption_key):
    ...
    with patch('chromadb.PersistentClient') as mock_client, \
         patch('chromadb.utils.embedding_functions.OpenAIEmbeddingFunction') as mock_ef:
...
def test_add_documents(secure_db):
    ...
    with patch.object(secure_db.client, 'get_collection') as mock_get:
        ...
        assert all(isinstance(next(iter(m.values())), str) 
                  for m in call_args["metadatas"])
...
def test_collection_not_found(secure_db):
    ...
    with patch.object(secure_db.client, 'get_collection', side_effect=ValueError("Collection not found")):
